...
- Contains two primary folders:
  - `extract/` → Stores raw input CSV files uploaded by users
  - `load/` → Stores transformed output files generated by AWS Glue
...
6. Click **Create folder** and create:
   - `extract/` → Input CSV files
   - `load/` → Transformed output files
...
### Targets
- Target 1: SNS Topic → `ETLJobNotification`
- Target 2: CloudWatch Log Group → `s3-object-upload-events`
...
### Targets
- SNS Topic → `ETLJobNotification`
- CloudWatch Log Group → `glue-job-status-change-events`
...
By integrating Amazon S3, AWS Lambda, AWS Glue, Amazon EventBridge, Amazon SNS, CloudWatch, and IAM, the solution achieves scalability, observability, reliability, and operational simplicity—making it suitable for both learning and real-world data engineering workloads.
